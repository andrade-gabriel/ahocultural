...
export async function getAsync(config: any, skip: number, take: number, name: string | null): Promise<CompanyIndex[]> {
    // Monta a URL para /_search do índice
    const base_path = `${config.elasticsearch.domain}/${config.elasticsearch.companyIndex}`;
...
export async function postAsync(config: any, doc: CompanyIndex): Promise<boolean> {
    if (!doc?.id)
