...
export const AdvertisementLayout = () => {
  ...
  useEffect(() => {
    ...
    getAdvertisement({ signal: ac.signal })
      ...
      .catch((e) => {
        if (!ac.signal.aborted)
          setErr(
            e instanceof Error
              ? e.message
              : t("advertisementLoadError", { defaultValue: "Falha ao carregar informações." })
          );
      })
      .finally(() => {
      ...
      <div className="mx-auto w-full max-w-[800px] px-6 py-10 prose prose-neutral dark:prose-invert">
        {html ? (
          ...
          <p className="text-muted-foreground">
            {t("emptyAdvertisement", { defaultValue: "Nenhuma informação disponível no momento." })}
          </p>
        )}
      </div>
    </div>
