...
import { getAboutById, insertAbout, updateAbout } from "@/api/about/actions";
...
export const AboutLayout = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getAboutById({ signal: ac.signal });
  ...
  const titleText = useMemo(() => "Quem Somos", []);
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values) => {
    try {
      ...
      if (hasExistingRef.current) await updateAbout(payload);
      else await insertAbout(payload);

      navigate("/about", { replace: true });
    } catch (e) {
      const msg = e instanceof Error ? e.message : "Falha ao salvar Quem Somos.";
      setError(msg);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Card className="shadow-sm">
      <CardHeader>
        ...
        <CardDescription>Gerencie o conteúdo multilíngua do “Quem Somos”</CardDescription>
      </CardHeader>
